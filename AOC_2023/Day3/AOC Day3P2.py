...
def solve(lines: list[str]) -> int:
    ...
    for position in grid.keys():
        ...
        # for stars, find the neighbors
        neighbor_number_ids = set()
        for neighbor in get_neighbours(position, grid):
            if neighbor in position_to_number_id.keys():
                neighbor_number_ids.add(position_to_number_id[neighbor])

        # if exactly two neighbors exist
        if len(neighbor_number_ids) == 2:
            near_neighbor_1, near_neighbor_2 = neighbor_number_ids
            gear_1 = numbers[near_neighbor_1].value
            gear_2 = numbers[near_neighbor_2].value
            
            Gear_Ratio  += gear_1 * gear_2
            
            print(f'PAIR AT {position} ->  G1: {gear_1} | G2: {gear_2} | Ratio: {gear_1 * gear_2} | SUM: {Gear_Ratio}')
